...
const controller = {
  async register(req, res) {
    if (!req.body.fullName || !req.body.login
	  ...
	  if (/\W/.test(req.body.login))
	    ...
	    user.profile_picture = "../public/images/default_avatar.png";
  ...
  async login(req, res) {
    ...
    if (authorized)
      res.status(200).json({message: "You are already authorized!"});
    else {
  ...
  async passwordReset(req, res) {
    if (!req.body.email)
	  ...
	  html: `<!doctype html> <html lang="en"> <head> <style>
		...
		<a href="/api/auth/password-reset/${token}">
		http://localhost:65535/api/auth/password-reset/${token}
		</a> </div> <p class="footer">Stomach Overflow 2025</p>
		</div> </body> </html>`
	}, (err, info) => {
	  if (err) {
	    console.log(err.message);
