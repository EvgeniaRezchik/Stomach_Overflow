...
const controller = {
  async getUsers(req, res) {
    ...
    if (tempUsers.length !== 0) {
      const pages = Math.ceil(tempUsers.length / limit);
      if (req.query.page) {
                ...
                if (user.login)
                  ...
                  if (user.email_address)
                    ...
                    if (/\W/.test(req.body.login))
                      ...
                      user.profile_picture = "../public/images"
                                             + "/default_avatar.png";
        ...
        if (payload) {
          ...
          if (req.user.id) {
            if (!req.file)
              ...
              req.user.profile_picture = "../public/images/"
                                         + req.file.filename;
    ...
    if (req.headers.authorization) {
      try {
        ...
        if (payload) {
          req.user = await new User().findOne(payload.id);
          if (req.user.id) {
            req.user.profile_picture = "../public/images/default_avatar.png";
            const result = await req.user.save();
            if (result !== null)
