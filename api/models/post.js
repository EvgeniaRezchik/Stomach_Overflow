...
module.exports = class Post extends Model {
  ...
  getDirectComments() {
    return Comment.findAll(this.id, "post_id", undefined, "ratings.rating",
                           "DESC", undefined, undefined, "(SELECT comment_id,"
                           + " max_likes.likes - max_likes.dislikes AS rating"
                           + " FROM (SELECT comment_id, max(likes) AS likes, "
                           + "max(dislikes) AS dislikes FROM (SELECT "
                           + "comment_id, CASE WHEN type = 'like' THEN count "
                           + "ELSE 0 END AS likes, CASE WHEN type = 'dislike'"
                           + " THEN count ELSE 0 END AS dislikes FROM (SELECT"
                           + " comment_id, type, count(type) AS count FROM "
                           + "(SELECT likes.type AS type, comments.id AS "
                           + "comment_id FROM likes RIGHT JOIN comments ON "
                           + "likes.comment_id = comments.id) AS types GROUP "
                           + "BY comment_id, type) AS counts) AS likes GROUP "
                           + "BY comment_id) AS max_likes) AS ratings",
